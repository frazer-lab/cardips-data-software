...
gwas_catalog = join(subdir, 'gwas_catalog.tsv')
...
gencode_tss_bed = join(subdir, 'gencode_v19', 'tss.bed')
kheradpour_motifs = join(subdir, 'kheradpour_motifs', 'motifs.txt')
picard_ref_flat = join(subdir, 'gencode_v19', 'gencode_no_rRNA.txt.gz')
picard_rrna = join(subdir, 'gencode_v19', 'rrna.interval')
roadmap_15_state = join(subdir, 'roadmap_15_state')
...
htslib = join(subdir, 'htslib-1.2.1', 'bin')
igvtools = join(subdir, 'IGVTools')
liftOver = join(subdir, 'liftOver')
